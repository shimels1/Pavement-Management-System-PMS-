...
import {
  RoadnetWorkInventoryCalculation,
  RoadnetWorkInventoryCalculationArray,
} from '../model/rodeInventoryCalculation';
...
@Component({
  selector: 'app-road-network-inventory',
  templateUrl: './road-network-inventory.component.html',
  styleUrls: ['./road-network-inventory.component.css'],
})
export class RoadNetworkInventoryComponent implements OnInit {
  rodeDialog: boolean = false;

  rodesRI: RoadnetWorkInventory[] = [];
  rodes: RoadnetWorkInventoryCalculation[] = [];
  rodes2: RoadnetWorkInventoryCalculationArray[] = [];
  ...
  constructor(
    private rodetService: RoadNetworkInventoryService,
    private PciSeverityService: PciSeverityService,
    private router: Router,
    private rodetServicepci: PciServiceService,
    private messageService: MessageService,
    private confirmationService: ConfirmationService,
    private modalService: NgbModal
  ) {}

  getAllData2(sectionID: any) {
    this.rodes = [];
    this.rodetServicepci.getBySectionID(sectionID + '').subscribe(
      (data) => {
        var i: number = 0;
  ...
  async ngOnInit() {
    ...
    this.items = [{ label: 'Pavement Inventory', routerLink: '/' }];
    this.home = { icon: 'pi pi-home', routerLink: '/' };

    this.getAllData();

    for (let i = 0; i < this.rodesRI.length; i++) {
      var sectionID = this.rodesRI[i].sectionID;
      setTimeout(function () {
  ...
  getAllData() {
    if (sessionStorage.getItem('role') != 'Admin')
      ...
      this.rodetService.getAllitems().subscribe((data) => {
        this.rodesRI = data;
        this.print(data);
      });
  }
  ...
  navigateToAddPCI(rode: RoadnetWorkInventory) {
    this.print(rode.sectionID);
    this.router.navigate(['/pci', rode.sectionID]);
  }
  deleteSelectedRodes() {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.rodesRI = this.rodesRI.filter(
  ...
  deleterode(rode: RoadnetWorkInventory) {
    console.log(rode.sectionID);
    this.rodetService.deleteOrder(rode.sectionID).subscribe(
      (res) => {
        console.log(res);
        this.getAllData();
      },
  ...
  saveRode(form: NgForm) {
    ...
    this.rodetService.saveRodeNetworkInventory(form.value).subscribe(
      (res) => {
        this.saveErrorPopup = '';
  ...
  findIndexById(id: string): number {
    let index = -1;
    for (let i = 0; i < this.rodesRI.length; i++) {
      if (this.rodesRI[i].sectionID === id) {
        index = i;
        break;
      }
    }
  ...
  openResultForFirsttime(rodes: RoadnetWorkInventoryCalculation[]) {
    var upper = 0;
    var lower: number = 0;
    rodes.forEach((element) => {
      element.SampleArea = 232.2576;
      upper = upper + element.PCI * element.SampleArea;
      lower = lower + parseFloat('' + element.SampleArea);
    });

    // this.print('final result');
    // this.print(upper)
    // this.print(lower)
    // this.print(upper/lower)
